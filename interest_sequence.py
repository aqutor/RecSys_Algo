...
def compute_lrsrs(seq, users, ratings, items, theta=0.2):
    # key: (user u,user v) val: |lrsrs(u,v)|
    lrsrs = {}
    ...
    return lrsrs, total, common_count, common_items


def compute_position(j, u, v, i, tuple_i, seq_v, ratings, theta):
    target_item = tuple_i[0]
...
def compute_arsrs(seq, users, ratings, theta=0.2):
    # key: (user u,user v) val: |arsrs(u,v)|
    arsrs = {}
    length = len(users)
    length = length * (length - 1) * 0.5
    comb = 0
    ...
    for u in users:
        arsrs[(u, u)] = 2 ** len(seq[u])
    return arsrs


def similarity_is(lrsrs, arsrs, users, total, common, alpha=1):
    length = len(users)
    length = length * (length - 1) * 0.5
    comb = 0
    sim = {}
    ...
    for u, v in combinations(users, 2):
        comb += 1
        if u != v:
            sim_lrsrs = lrsrs[(u, v)] / (math.sqrt(lrsrs[(u, u)] * lrsrs[(v, v)]))
            sim_arsrs = arsrs[(u, v)] / (math.sqrt(arsrs[(u, u)]) * math.sqrt(arsrs[(v, v)]))
            sim[(u, v)] = alpha * np.exp(sim_lrsrs) + (1 - alpha) * np.exp(sim_arsrs)
...
def similarity(weighted_sim, users, seq, common_items, item_ratings):
    ...
    length = length * (length - 1) * 0.5
    comb = 0
    # key: (user u,user v) val: similarity based on interest sequence * pearson correlation
...
def precision_recall_calculation(predictions, threshold=3.5):
    ...
    for user_id, user_ratings in user_predict_true.items():
        ...
        no_of_recommended_items = sum(
            (predicted_rating >= threshold) for (predicted_rating, true_rating) in user_ratings[:10])

        # Number of relevant and recommended items in top 10
        no_of_relevant_and_recommended_items = sum(
            ((true_rating >= threshold) and (predicted_rating >= threshold)) for (predicted_rating, true_rating) in
            user_ratings[:10])

        # Precision: Proportion of recommended items that are relevant
        precisions[
            user_id] = no_of_relevant_and_recommended_items / no_of_recommended_items if no_of_recommended_items != 0 else 1

        # Recall: Proportion of relevant items that are recommended
        recalls[
            user_id] = no_of_relevant_and_recommended_items / no_of_relevant_items if no_of_relevant_items != 0 else 1
...
def interest_sequence(df_train, df_test, mode, alpha, K):
    ...
    if mode == "Train":
        ...
        # compute lrsrs values between every pair of users
        print("\nComputing lrsrs")
        lrsrs, total, common_count, common_items = compute_lrsrs(seq, users, ratings, items)

        # storing the results for use in the future
        file = open('store/lrsrs.txt', 'wb')
        pickle.dump(lrsrs, file)
        file.close()
        ...
        # computing arsrs values between every pair of users
        print("\nComputing arsrs")
        arsrs = compute_arsrs(seq, users, ratings)
        # storing the array
        file = open('store/arsrs.txt', 'wb')
        pickle.dump(arsrs, file)
        file.close()
        print("Elapsed Time: ", time.time() - start_time)

        # computing similarity between every pair of users based on lrsrs and arsrs
        print("\nComputing Similarity Weights")
        sim_is, weighted_sim = similarity_is(lrsrs, arsrs, users, total, common_count, alpha)
    ...
    if mode == "Test":
        ...
        file = open('store/lrsrs.txt', 'rb')
        lrsrs = pickle.load(file)
        file.close()
        ...
        file.close()
        file = open('store/arsrs.txt', 'rb')
        arsrs = pickle.load(file)
        file.close()
